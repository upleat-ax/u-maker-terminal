...
- **SSH 원격 접속** — 호스트 저장·**편집** + 키/패스워드 인증, **vi · tmux 풀스크린에서도 사이즈 깨짐 없음**. 패스워드 연결을 **키 인증으로 변환**(키 자동 생성·원격 설치·다운로드)하고, 저장된 전체 접속정보를 **passphrase 기반 AES-256 암호화 파일로 내보내기** 지원
- **Git 저장소 바로 열기** — StatusBar 브랜치 옆 버튼으로 `origin` 원격 주소를 브라우저에서 즉시 열기
